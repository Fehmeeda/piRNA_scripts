...
def load_fold_features(species, fold, kmertype, decision_type):
    ...
    return  X_train, H_train,y_train, X_test,H_test, y_test

def create_train_val_split( X, H,y, val_ratio=0.2):
    Xtr, Xval, Htr, Hval, ytr, yval = train_test_split(
        X, H, y,
        test_size=val_ratio,
        stratify=y,
        random_state=42
    )
    return Xtr, Htr, ytr, Xval, Hval, yval
...
class FusionNet(nn.Module):
    ...
    def __init__(self, input_shape,hdv_len, dropout=0.5):
        super().__init__()

        # Weighted CNN branch
        self.conv_w1 = nn.Conv2d(1, 32, kernel_size=(6,6), padding=3)
        ...
        self.bn_h = nn.BatchNorm1d(hdv_len)
...
for fold in FOLDS:
    ...
    Xtr, Htr, ytr, Xte, Hte, yte = load_fold_features(
    SPECIES, fold, KMERTYPE, DECISION_TYPE
    ) 

    # -------- create validation split --------
    #Xtr, Dtr, Htr, ytr, Xval, Dval, Hval, yval = create_train_val_split(Xtr, Dtr, Htr, ytr)
    Xtr, Htr, ytr, Xval, Hval, yval = create_train_val_split(Xtr,Htr, ytr)

    train_ds = FusionDataset( Xtr, Htr, ytr)
    val_ds   = FusionDataset(Xval, Hval, yval)
    test_ds  = FusionDataset( Xte, Hte, yte)
    ...
    model = FusionNet(input_shape=input_shape, hdv_len=hdv_dim).to(DEVICE)
