...
# =======================================================
# 4. Compute PMF (Probability Mass Function)
# =======================================================
def compute_pmf(sequences, all_kmers, k=3):
    counts = Counter()
    ...
    pmf = [counts[k] / total for k in all_kmers]
    return pmf
...
def plot_heatmap(all_kmers, pdf_pos, pdf_neg, species):
    ...
    plt.title(f"{species}: Overlapping 3-mer Probability Heatmap")
    plt.tight_layout()
    plt.show()
...
def process_all_species(data_dir="Datasets"):
    ...
    for species, (pos_file, neg_file) in species_files.items():
        ...
        pmf_pos = compute_pmf(pos_seqs, all_kmers)
        pmf_neg = compute_pmf(neg_seqs, all_kmers)
