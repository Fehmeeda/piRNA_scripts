...
# =============================
BASE_OUTDIR = "SVM_dna2vec_Results_rbf"
...
for species, file_path in species_files.items():
    ...
    # Save text results
    # -----------------------------
    with open(os.path.join(outdir, "svm_dna2vec_report.txt"), "w") as f:
        f.write(f"{species} — SVM on dna2vec vector\n")
        f.write("=" * 45 + "\n")
    ...
    plt.title(f"{species} — ROC (dna2vec + SVM)")
    plt.legend()
    plt.tight_layout()
